...
export type GenerationPersistMedium = "cloud" | "local" | "session" | "mysql";

/**
 * AI 命题落盘顺序。
 * - local：仅本地 JSON → 会话
 * - builtin：本地 JSON → 云端 → MySQL → 会话（MySQL 不可用时跳过）
 * - auto / supabase：云端 → MySQL → 本地 JSON → 会话
 */
export function generationPersistOrder(
  ...
  if (pref === "builtin") return ["local", "cloud", "mysql", "session"];
  return ["cloud", "mysql", "local", "session"];
}
