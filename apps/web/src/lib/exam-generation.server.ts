...
function normalizeQuestionAliases(raw: Record<string, unknown>): ParsedAiQuestion {
  ...
  if (!content && (qType === "multiple_choice" || qType === "multiple_choice_multi")) {
...
export function canonicalizeImportedExamPayload(parsed: Record<string, unknown>): Record<string, unknown> {
  ...
  const out: Record<string, unknown> = {
    ...root,
    questions: normalizedQs as unknown[],
  };
  for (const k of ["problems", "items", "question_list", "exam_questions"] as const) {
    if (k in out) delete out[k];
  }
  if (out.exam && typeof out.exam === "object" && !Array.isArray(out.exam)) {
    const ex = { ...(out.exam as Record<string, unknown>) };
    for (const k of ["questions", "problems", "items", "question_list", "exam_questions"]) {
      delete ex[k];
    }
    if (Object.keys(ex).length === 0) delete out.exam;
    else out.exam = ex;
  }
  return out;
}
...
/** 从文末向前：最后一个能串起 A→B→C→D 的 (A) 作为选项区起点（避开几何图内重复的 (A)(B) 标注） */
function findLastOrderedParenChain(
  hits: ParenLetterHit[],
  letters: string[],
): ParenLetterHit[] | undefined {
  if (hits.length < letters.length) return undefined;
  const first = letters[0];
  if (!first) return undefined;
  const starters = hits.filter((h) => h.letter === first).sort((a, b) => b.idx - a.idx);
  for (const start of starters) {
    const chain: ParenLetterHit[] = [start];
    let minNext = start.idx + start.headerLen;
    let ok = true;
    for (let li = 1; li < letters.length; li++) {
      const L = letters[li]!;
      const next = hits
        .filter((h) => h.letter === L && h.idx >= minNext)
      ...
      chain.push(next);
      minNext = next.idx + next.headerLen;
    }
    if (ok && chain.length === letters.length) return chain;
  }
  return undefined;
}

/** 去掉选项正文末尾误并入的下一轮括号头（OCR 粘连） */
function sanitizeParenOptionBody(text: string): string {
  return text.replace(/\s*[（(]\s*[A-Ha-h]\s*[）)]\s*$/, "").trim();
}

function buildStemAndOptionsFromParenChain(
  s: string,
  chain: ParenLetterHit[],
): { stem: string; options: string[] } {
  const options: string[] = [];
  for (let i = 0; i < chain.length; i++) {
    const { letter, idx, headerLen } = chain[i]!;
    const textStart = idx + headerLen;
    const textEnd = i + 1 < chain.length ? chain[i + 1]!.idx : s.length;
    let text = s.slice(textStart, textEnd).trim().replace(/\s+/g, " ");
    text = sanitizeParenOptionBody(text);
    options.push(`${letter}. ${text}`);
  }
  const stem0 = chain[0] ? s.slice(0, chain[0].idx).trim() : s.trim();
  return { stem: stem0 || s.trim(), options };
}

/**
 * 英语听力 / 几何卷常见 (A) … (B) … (C) … (D) …（括号+字母）。
 * 旧实现按出现顺序切片，图中顶点 (A)(B) 与文末选项混在一起会产生 `(A) 2 (B) 3 … (A) 0` 类乱序；
 * 改为优先匹配文末完整的 A→B→C→D（必要时 A→…→E）。
 */
function extractParenLetterMcqOptions(
  ...
  const chain5 = findLastOrderedParenChain(hits, ["A", "B", "C", "D", "E"]);
  if (chain5) {
    const built = buildStemAndOptionsFromParenChain(s, chain5);
    return built.options.length >= 4 ? built : undefined;
  }

  const chain4 = findLastOrderedParenChain(hits, ["A", "B", "C", "D"]);
  if (chain4) {
    return buildStemAndOptionsFromParenChain(s, chain4);
  }
...
function normalizeSingleMcqQuestion(q: ParsedAiQuestion): ParsedAiQuestion {
  ...
  const collectLabeledOptionLines = (blob: string): string[] | undefined => {
    const lines = blob
      .split(/\r?\n/)
      .map((l) => l.trim())
      .filter(Boolean);
  ...
  const splitBlobIntoLabeledParts = (chunk: string): string[] | undefined => {
    ...
    const fromLines = collectLabeledOptionLines(trimmed);
    if (fromLines) return fromLines;

    const fromParenOnly = extractParenLetterMcqOptions(trimmed);
...
async function runExamAiPlainJsonFallback(
  config: GenerationConfig,
): Promise<Record<string, unknown>> {
  const composition = config.composition
    .filter((c) => c.count > 0)
 ...
 */
async function runExamAiGenerationResilient(
  config: GenerationConfig,
): Promise<Record<string, unknown>> {
  const errors: string[] = [];
...
async function mergeCompositionSegmented(
  config: GenerationConfig,
): Promise<Record<string, unknown>> {
...
字段：title（string）、subtitle（string）、description（string）、questions（array，须非空）。
questions 每元素含：type、subject、points、content、options（选择题须至少 4 个字符串，否则 null）、answer、solution_steps（至少 2 步，每步含 step、description、reasoning）、knowledge_tags（字符串数组）。
multiple_choice / multiple_choice_multi：**options 须为≥4 条的字符串数组**，每条对应一个选项；禁止仅用两条字符串代替四个选项。
    ...
    messages: [
      {
        role: "system",
        content:
          "你只输出一个合法 JSON 对象本身，不得输出任何其它字符。键名使用双引号。必须包含 title、subtitle、description、questions（非空数组）。",
      },
      { role: "user", content: userPrompt },
    ],
    max_tokens: resolveExamAiMaxOutputTokens(2),
...
- 线段 AB、AC、AM 易被误成「48」「4B」「4C」「4M」；A 单独易被误成「4」（如「若 AD=12」误成「若 41=12」）。
...
${body}`;
  ...
  try {
    ...
    throw new Error(
      `未能将文档整理为结构化试卷（无 submit_exam）。${buildMissingToolCallDetail(data)} 已自动尝试「纯 JSON」回退但仍失败：${fb}`,
    );
  }
}
...
export type ImportDocumentHints = {
  ...
  duration_min?: number;
  /** 与命题页试卷场景 id 一致（如 regular_final）；缺省为 regular_daily */
  paper_kind?: string;
  /** 网上导入等待确认的临时卷 */
...
export function buildImportedExamSnapshotFromAiParsed(
  ...
  const pkHint = hints?.paper_kind?.trim();
  const paperKindResolved =
    pkHint &&
    [
      "regular_daily",
      "regular_unit",
  ...
  const exam: Exam = {
    ...
    ...(hints?.import_review_status === "staging"
      ? { import_review_status: "staging" as const }
      : {}),
  };
...
function attachCompositionTypeLabels(
  qs: Question[],
  composition: CompositionRowPayload[],
): Question[] {
          ...
          items: {
            ...
            properties: {
              ...
              options: {
                ...
                items: { type: "string" },
                description:
                  "仅 multiple_choice / multiple_choice_multi：至少 4 个备选项，可为更多",
              },
              answer: {
                type: "string",
                minLength: 1,
...
function extractToolPayloadFromAssistantContent(
  ...
  /** 首个花括号平衡片段（避免 lastIndexOf('}') 对齐到内层或尾部说明文字导致整段无法 parse） */
  const balancedOuter = extractFirstBalancedJsonSegment(stripped, "{");
  const candidates: string[] = [];
  const pushCandidate = (s: string) => {
    const t = s.trim();
    if (t.length && !candidates.includes(t)) candidates.push(t);
  };
  if (balancedOuter) pushCandidate(balancedOuter);
  pushCandidate(stripped);
  const lo = stripped.indexOf("{");
  const hi = stripped.lastIndexOf("}");
  if (lo >= 0 && hi > lo) {
    pushCandidate(stripped.slice(lo, hi + 1));
  }
  ...
  try {
    ...
    if (persistedQs?.length) {
      await writeListeningScriptMarkdownForEnglishListeningExam(
        examIdStr,
        {
          title: String(examRow.title ?? ""),
          subjects: (examRow.subjects ?? []) as Exam["subjects"],
        },
    ...
    console.warn(
      "[listening-script] 命题入库后写入 listening-script.md 失败（试卷已保存）:",
